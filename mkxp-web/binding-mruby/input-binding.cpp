...
/* WEB PORT: raw scancode probe so the Ruby Input module can implement the
 * Windows-virtual-key text-entry API (Input.repeatex?/triggerex?/pressex? etc.)
 * on top of EventThread::keyStates, which the JS keyboard listener feeds via
 * web_set_scancode. Returns true if the given SDL scancode is currently down. */
extern "C" int web_get_scancode(int scancode);
MRB_FUNCTION(inputWebScancode)
{
	MRB_FUN_UNUSED_PARAM;
	mrb_int sc;
	mrb_get_args(mrb, "i", &sc);
	return mrb_bool_value(web_get_scancode((int)sc) > 0);
}
#endif
...
#ifdef __EMSCRIPTEN__
	mrb_define_module_function(mrb, module, "web_scancode?", inputWebScancode, MRB_ARGS_REQ(1));
#endif
