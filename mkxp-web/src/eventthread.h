...
public:
	...
	/* WEB PORT: per-frame SDL event drain for emscripten (keyboard/mouse/joy ->
	 * keyStates/mouseState/joyState). process() only runs once at startup in the
	 * single-threaded web build, so input must be pumped every frame from the
	 * main loop. */
	static void processEmscriptenEvents();
