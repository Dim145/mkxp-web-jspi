...
// Default RGSS internal render resolution: stock RMXP/RGSS1 is 640x480 (RGSS2/VX is
// 544x416). A game authored for a different internal resolution should call
// Graphics.resize_screen(w, h) at boot (the standard RGSS API) — no engine rebuild
// needed. The rendered frame is scaled to the window/canvas by the browser.
// NOTE: some RGSS1 games (e.g. Pokémon Essentials) assume a non-default resolution
// without calling resize_screen; for those, either add a resize_screen call during
// porting or set defScreenW/defScreenH in the game config.
...
#ifdef __EMSCRIPTEN__
		/* WEB PORT: in the browser, a single frame can stall for SECONDS on an
		 * async asset load (XHR through ASYNCIFY). The unbounded integration
		 * above would turn that into idealDiff = <stall duration>, making
		 * frameSkipRequired() true for hundreds of consecutive frames: the
		 * screen freezes while game logic fast-forwards — the "0.5 fps"
		 * slideshow. Frame debt beyond a few frames is unrecoverable jitter,
		 * not something to catch up on; drop it. */
		if (adj.idealDiff > 4 * (int64_t)tpf || adj.idealDiff < -4 * (int64_t)tpf)
			adj.idealDiff = 0;
#endif
...
private:
	...
	{
#ifdef __EMSCRIPTEN__
		/* WEB PORT: plain SDL_Delay busy-waits and never returns control to the
		 * browser, so the canvas never repaints and queued key/timer events never
		 * fire. emscripten_sleep() (ASYNCIFY) unwinds the ENTIRE C+Ruby call stack
		 * — including any intervening C frames such as Proc#call — hands control to
		 * the browser event loop for the requested time, then rewinds and resumes
		 * right here. This is what lets RGSS games' blocking `loop { Graphics.update }`
...
#ifdef __EMSCRIPTEN__
	/* WEB PORT: the frame limiter must never be fully disabled, because its
	 * delay() is the single point where Graphics.update yields to the browser
	 * (via emscripten_sleep). A disabled limiter would spin the game loop
	 * forever and hard-freeze the tab. Force it on at a sane rate. */
	if (p->fpsLimiter.disabled)
	{
		p->fpsLimiter.disabled = false;
		p->fpsLimiter.setDesiredFPS(p->frameRate > 0 ? p->frameRate : 60);
	}
#endif
}
...
void Graphics::update()
{
...
#ifdef __EMSCRIPTEN__
	/* WEB PORT: on native, every ALStream owns a background thread that keeps its
	 * OpenAL buffers refilled. The single-threaded Emscripten build has no such
	 * thread, so streaming buffers are only refilled when shState->audio().update()
	 * runs. That call used to live *only* in Graphics::transition(), so BGM/BGS/ME
	 * played their initial STREAM_BUFS buffers and then starved to silence during
	 * normal gameplay -- while SE (a single fully-loaded buffer, no refill needed)
	 * kept working. Pump the streams here every frame so music actually plays.
	 * Placed before the frozen / frame-skip early-outs so audio never starves even
	 * while the screen is frozen or rendered frames are being skipped. */
	...
	{
#ifdef __EMSCRIPTEN__
		/* Still hand control back to the browser while the screen is frozen,
		 * otherwise a `Graphics.freeze; loop { Graphics.update }` pattern would
		 * spin without ever repainting or processing input. */
		emscripten_sleep(1);
#endif
		return;
	}
