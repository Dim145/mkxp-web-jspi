...
#define PO_DESC_ALL \
	 ...
	 * the draw-quad fallback — leave blitting enabled. */
	enableBlitting = true;
	winResizable = false;
	windowTitle = "MKXP";
	/* WEB PORT: was `frameSkip = true`. In the browser, any async asset load
	 * can stall a frame for seconds; with frameSkip on, the limiter then skips
	 * drawing for hundreds of frames to "catch up" (screen frozen, logic
	 * fast-forwarding). Always draw; Graphics::update's else-branch resets the
	 * frame adjust instead. */
	frameSkip = false;
