...
struct SoundOpenHandler : FileSystem::OpenHandler
{
	...
	bool tryRead(SDL_RWops &ops, const char *ext, const char *fullPath)
	{
		...
		/* WEB PORT: SDL_sound is compiled out; dispatch by file signature to the
		 * in-memory decoders (OGG/WAV/MP3). The create* helpers close `ops` on
		 * error, so a failure just means "not this format" -> return false. */
		char sig[5] = { 0 };
		SDL_RWread(&ops, sig, 1, 4);
		SDL_RWseek(&ops, 0, RW_SEEK_SET);

		ALDataSource *source = 0;
		try
		{
			if (!strcmp(sig, "OggS"))
				source = createVorbisSource(ops, false);
			else if (!strncmp(sig, "RIFF", 4))
				source = createWavSource(ops, false);
			else if (!strncmp(sig, "ID3", 3) ||
			         ((unsigned char)sig[0] == 0xFF && ((unsigned char)sig[1] & 0xE0) == 0xE0))
				source = createMp3Source(ops, false);
			else
			{
				/* Unknown signature: fall back to Vorbis (also closes ops on error). */
				source = createVorbisSource(ops, false);
			}
		}
