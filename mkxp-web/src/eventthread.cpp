...
/* WEB PORT: sync SDL's live input state into mkxp's keyStates each frame.
 * The single-threaded web build never re-runs process(), so keyStates would
 * stay frozen. We POLL SDL_GetKeyboardState/SDL_GetMouseState (which the
 * Emscripten SDL2 backend updates directly on browser key/mouse events) rather
 * than draining the SDL event queue with SDL_PollEvent — draining from inside
 * the emscripten main-loop callback deadlocked the loop. */
void EventThread::processEmscriptenEvents()
{
	int numkeys = 0;
	...
	mouseState.x = mx;
	mouseState.y = my;
	mouseState.buttons[SDL_BUTTON_LEFT]   = (mb & SDL_BUTTON(SDL_BUTTON_LEFT))   != 0;
	mouseState.buttons[SDL_BUTTON_MIDDLE] = (mb & SDL_BUTTON(SDL_BUTTON_MIDDLE)) != 0;
	mouseState.buttons[SDL_BUTTON_RIGHT]  = (mb & SDL_BUTTON(SDL_BUTTON_RIGHT))  != 0;
}
