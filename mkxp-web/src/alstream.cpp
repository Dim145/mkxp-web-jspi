...
#ifdef __EMSCRIPTEN__
			/* WEB PORT: the SDL_sound streaming path below is compiled out on
			 * Emscripten, so WAV/MP3 music would be silent. Decode them wholesale
			 * into in-memory PCM (SDL for WAV, minimp3 for MP3) instead. */
			if (!strncmp(sig, "RIFF", 4))
			{
				source = createWavSource(*srcOps, looped);
				return true;
			}
			if (!strncmp(sig, "ID3", 3) ||
			    ((unsigned char)sig[0] == 0xFF && ((unsigned char)sig[1] & 0xE0) == 0xE0))
			{
				source = createMp3Source(*srcOps, looped);
				return true;
			}
#endif
