...
int rgssThreadFun(void *userdata)
{
	...
	/* WEB PORT: without an explicit version request, SDL's Emscripten backend
	 * creates a WebGL 1.0 context (GLES2). That leaves gl.BlitFramebuffer
	 * unresolvable (WebGL1 has no blit) and gl.unpack_subimage false (forcing a
	 * CPU SDL_BlitSurface staging copy on every clipped texture upload) — both
	 * major slowdowns. Request GLES 3.0 → WebGL 2.0 (the link already passes
	 * -sMAX_WEBGL_VERSION=2); fall back to the default (WebGL1) context if the
	 * browser can't provide one. */
	...
	if (!glCtx)
	{
		Debug() << "WebGL2 context unavailable, falling back to WebGL1:" << SDL_GetError();
